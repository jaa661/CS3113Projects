...
bool testSATSeparationForEdge(float edgeX, float edgeY, const std::vector<V3> &points1, const std::vector<V3> &points2, V3 &penetration) {
    float normalX = -edgeY;
    float normalY = edgeX;
...
bool penetrationSort(V3 &p1, V3 &p2) {
    return p1.length() < p2.length();
}

bool checkSATCollision(const std::vector<V3> &e1Points, const std::vector<V3> &e2Points, V3 &penetration) {
    std::vector<V3> penetrations;
...
public:
    ...
    void DrawSpriteSheetSprite(ShaderProgram *program, int index, std::vector<V3> points);
...
void SheetSprite::DrawSpriteSheetSprite(ShaderProgram *program, int index, std::vector<V3> points) {
    ...
    float vertices[] = {points[3].x, points[3].y, points[1].x, points[1].y, points[2].x, points[2].y, points[1].x, points[1].y,  points[3].x, points[3].y, points[0].x, points[0].y};
...
class Entity{
    ...
    fired = 0;
        for(int i = 0; i <poly; i++){
            V3 temp;
            temp.x = position.x + cos(0.0174533*i*(360/poly));
            temp.y = position.y + sin(0.0174533*i*(360/poly));
            temp.z = 0;
...
void Entity::draw(ShaderProgram *program){
    ...
    program->setModelMatrix(modelMatrix);
    sprite.DrawSpriteSheetSprite(program, index, points);;
...
void Entity::update(float elapsed){
    ...
    for(int i = 0; i <poly; i++){
        points[i].x += velocity.x * elapsed;
        points[i].y += velocity.y * elapsed;
    }
    accel.x = 0;
    accel.y = 0;
...
public:
    floating(){
        ...
        for(int i = 0; i <poly; i++){
            V3 temp;
            temp.x = position.x + cos(0.0174533*i*(360/poly) + 90);
            temp.y = position.y + sin(0.0174533*i*(360/poly) + 90);
            temp.z = 0;
            points.push_back(temp);
        }
    }
...
void init(grounded &player, GameState &state, std::vector<floating> &entities, Entity tiles[100][100]){
    ...
    for(int i = 0; i < 100;i++){
        //for(int j = 0; j < 100;j++){
            ...
            e.solid = true;
            e.rotation = random;
            e.velocity.x = random *cos(random);
            e.velocity.y = random *sin(random);
            //std::cout<<random *cos(random)<<"xxx"<<std::endl;
            e.index = 20;
            e.textureID = spriteSheetTexture;
            e.sprite = mySprite;
            entities.push_back(e);
...
void UpdateGameLevel(int &modifier, grounded &player, std::vector<floating> &entities, float elapsed, GameState &state){
    ...
    for(int i=0; i < entities.size(); i++) {
        ...
        V3 penetration;
        if(checkSATCollision(entities[i].points, player.points, penetration)){
            entities[i].position.x += .05* penetration.x;
            entities[i].position.y += .05*penetration.y;
        }
        cout<<checkSATCollision(player.points, entities[i].points, penetration)<<endl;
    }
    if (SCORE >=48)
        state = WIN;
